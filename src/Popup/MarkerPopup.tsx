...
import { PopupType } from "./Type/PopupType";
    ...
    <>
      ...
      <div className="popup-detail popup-detail-custome">
        {marker?.Shops?.map((good,index) => (
          <div key={`Shops${index}`} className="popup-row mt-2 popup-row-custom">
        ...
        {marker?.Products?.map((good,index) => (
          <div  key={`Products${index}`} className="popup-row mt-2 popup-row-custom">
            <span
